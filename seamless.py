...
def find_matches(template, sample_image, valid_mask, gauss_mask, tot_weight):
    ...
    ssd_map = np.zeros_like(sample_image, dtype=np.float32)

    pad = window_size // 2
    padded_sample = np.pad(sample_image, pad, mode='reflect')

    # Iterate over each possible patch in sample image
    for i in range(h):
        for j in range(w):
            patch = padded_sample[i:i + window_size, j:j + window_size]
            diff = (template - patch) * valid_mask
            weighted_ssd = np.sum((diff ** 2) * gauss_mask * valid_mask)
            if tot_weight > 0:
                ssd_map[i, j] = weighted_ssd / tot_weight
            else:
                ssd_map[i, j] = float('inf')
...
def get_unfilled_neighbors(image):
    ...
    """
    filled = (image > -1).astype(np.uint8)  # Assume -1 is "unfilled"
    unfilled = (image == -1).astype(np.uint8)
    ...
    def filled_neighbor_count(p):
        x, y = p
        neighbors = image[max(0, y - 1):y + 2, max(0, x - 1):x + 2]
        return np.sum((neighbors != -1) & (neighbors != 255))  # Assuming 255 is not used here
...
def grow_image(sample_image, image, window_size):
    ...
    iteration = 0
    total_pixels = np.sum(image == -1)  # Count unfilled pixels
    ...
    while True:
        ...
        remaining_pixels = np.sum(image == -1)
        filled_pixels = total_pixels - remaining_pixels
        progress_percent = (filled_pixels / total_pixels) * 100
        elapsed_time = time.time() - start_time
        print(f"Processing: {progress_percent:.2f}% complete, {remaining_pixels} pixels remaining, time: {elapsed_time:.2f}s")

        progress = False
        for x, y in pixel_list:
            template = get_neighborhood_window(image, (x, y), window_size)
            valid_mask = (template != -1).astype(float)
            ...
            matched_value = np.random.choice(matches)
        ...
        cpu_percent = psutil.cpu_percent(interval=0.1)  # Overall CPU usage
        print(f"Image growth completed in {total_time:.2f} seconds")
        print(f"Final memory usage: {mem_usage:.2f} MB, System CPU: {cpu_percent:.1f}%")
    total_time = time.time() - start_time
    print(f"Image growth completed in {total_time:.2f} seconds")
    return image